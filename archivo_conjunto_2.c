...
float media(double* Vector);
//float media1(double* Vector);
float varianza(double* Vector);
//float varianza1(double* Vector);
float engtot(double* Vector);
void regresion(double* Vector);
...
float varianza(double* Vector)
{
  ...
  for(i=0 ; i<tamanio ; i++)
  {
    sum = sum + ((Vector[i]-media(Vector)) * (Vector[i]-media(Vector)));
  }
...
float engtot(double* Vector) //energia total generada a lo largo del tiempo
{
  int tamanio;
  tamanio = sizeof(Vector) / sizeof(Vector[0]);
  int i;
...
void regresion(double* Vector)//calcula la recta de regresion de una fila de datos usando la media la varianza y la covarianza(hay que hacer la covarianza aun) para usando esa recta hacer una estimacion de lo que podria ser un valor futuro
{//en regresion hay que introducir tres datos , el primer vector (y) es el valor energético , el segundo vector son los dias o fechas en los que ocurren esos valores , las fechas se hacen como numeros enteros para facilitar los calculos .
  ...
  int i;
  double covarianza , sum=0;
  for(i=0 ; i<tamanio ; i++)
  {
    sum = sum + ((Vector[i]-media(Vector)) * (vectorFecha[i]-media(vectorFecha)));
  }
  covarianza = sum / tamanio;
  
  b = (covarianza)/(varianza(vectorFecha));
  printf("La recta de regresion tiene la forma : y= %f + %f * (x-%f) , ahora , dame un numero entero positivo , ese numero representara la cantidad de meses despues del ultimo mes registrado y hara una estimacion para ese mes. Pero cuanto mayor sea el numero , mayor sera el error\n", media(Vector), b, media(vectorFecha));
  int num_meses;
  scanf("%i",&num_meses);
  float error ,estimacion;
  
  estimacion = media(Vector) + b * (((tamanio)+num_meses)-media(vectorFecha));  
  error = covarianza / (varianza(Vector) * varianza(vectorFecha));
