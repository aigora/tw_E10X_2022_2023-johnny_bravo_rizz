...
float engtot(struct datosMatriz *datosATrabajar);
void regresion(struct datosMatriz *datosATrabajar);
...
{
  int i;/*valor unico*/
  float j = datosATrabajar->vectorFila[0], resultado;/*sumatorio de los valores*/
  for(i=0; i<datosATrabajar->numColumnas ; i++)
  {
	j=j+datosATrabajar->vectorFila[i];
  }
  resultado = (j)/(datosATrabajar->numColumnas);/*n es el tamaño del vector , osea , el numero de valores*/
}return resultado;
